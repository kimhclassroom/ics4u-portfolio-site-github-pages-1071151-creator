# Project overview
  A simple solo mini pong game

# The project includes:
- One platform that is controlled with keyboard.  
- A moving ball.
- A score.
  
# What the project does
- The ball moves, changing direction when touching the edges of the screen or the platform. 
- The platform can move left and right, and is controlled by the keyboard.
- Each time the ball touches the platform score is incrimented by 1.
- The game ends when the ball reaches the bottom of the screen.
- The game be restarted with a press of a "start" button.

# How to run it
- Press start button.
- Enter a name.

# Goal
- Get the highest score.
